...
renderer.link = (href: string, title: string, text: string) => {
  return `<a href="${href}" title="${title || ""}" target="__blank" rel="noopener"/>${text}</a>`;
};
...
const Markdown = ({ markdown, repoID }: { markdown: string; repoID: string }) => {
  const classes = useStyles();
  const html = useMemo(() => marked(replaceShortcuts(markdown, repoID), { renderer }), [markdown, repoID]);
