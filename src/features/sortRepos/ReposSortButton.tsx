...
export const ReposSortButton = ({ order, setReposSortOrder }: Props) => {
  ...
  return (
    <Tooltip title="Select sort order">
      <span>
        <IconButton aria-haspopup="true" aria-controls="order-menu" aria-label="sort order" onClick={onClick}>
          {options[selectedIndex].icon}
        </IconButton>
        <Menu id="order-menu" anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={handleClose} variant="selectedMenu">
