...
const reposSlice = createSlice({
  ...
  reducers: {
    ...
    updateRepoFailed(state, action: PayloadAction<{ id: string; error: Error }>) {
...
export const addNewRepo = (repoID: string): AppThunk => async (dispatch, getState) => {
  if (!getState().online) {
    return;
  }
  ...
  try {
    ...
    dispatch(addNotification({ message: `Added "${repoID}"`, type: "success" }));
  } catch (err) {
    const msg = err instanceof RepoNotFoundError ? err.message : `Error adding "${repoID}"`;

    dispatch(addNotification({ message: msg, type: "error" }));
  }
};

export const updateLatestRelease = (repoID: string): AppThunk => async (dispatch, getState) => {
  if (!getState().online) {
    return;
  }
  try {
 ...
 */
const updateInterval = parseInt(process.env.REACT_APP_UPDATE_INTERVAL || "1800") * 1000;
